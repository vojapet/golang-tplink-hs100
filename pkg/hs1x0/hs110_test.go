...
var _ = Describe("Hs110", func() {
	const anIpAddress = "192.168.2.1"
	const aDeviceName = "some-device-name"

	Describe("turnOn", func() {
		const turnOnQuery = `{"system":{"set_relay_state":{"state":1}}}`
		It("sends turn on command", func() {
			turnOnResponse := `{"system":{"set_relay_state":{"err_code":0}}}`
			mockExecutor := MockExecutor{}

			mockExecutor.MockExecute = func(aQuery commands.Query) (commands.Response, error) {
				b, _ := json.Marshal(aQuery)

				Expect(string(b)).To(Equal(turnOnQuery))

				response := make(commands.Response)
				err := json.Unmarshal([]byte(turnOnResponse), &response)
				if err != nil {
					return nil, err
				}
				return response, nil
			...
			hs110 := hs1x0.NewHs110(anIpAddress)
			hs110.CommandExecutor = &mockExecutor

			err := hs110.TurnOn()

			Expect(err).NotTo(HaveOccurred())
			assertOneCommandSend(anIpAddress, turnOnQuery)
		})
		...
		It("fails if sending command failed", func() {
			...
			mockExecutor.MockExecute = func(aQuery commands.Query) (commands.Response, error) {
				return commands.Response{}, errors.New("error")
			}
		...
		It("fails if error code is non zero", func() {
			turnOnResponse := `{"system":{"set_relay_state":{"err_code":-3}}}`
		...
		It("fails if response is invalid", func() {
			turnOnResponse := `{"system":{"set_reEEEEElay_state":{"err_code":-3}}}`
...
type ExecuteFunction func(aQuery commands.Query) (commands.Response, error)


type MockExecutor struct {
	MockExecute ExecuteFunction
}

func (self *MockExecutor) GetIp() string {
	return ""
}

func (self *MockExecutor) GetTimeout() time.Duration {
	return 1 * time.Second
}

func (self *MockExecutor) Execute(aQuery commands.Query) (commands.Response, error) {
	if self.MockExecute != nil {
		return self.MockExecute(aQuery)
	} else {
...
// }
//
func assertOneCommandSend(address string, command string) {
